...
def score_rotation_candidates(
    ...
    """Score and rank current Top-10 candidates using the four-factor model.

    The current cohort is included as the newest persistence cohort. Pass up to
    five prior cohorts in ``historical_cohorts`` to form the six-month window.
    """
    ...
    scored = [_score_candidate(row, persistence, aligned_sectors, breakout_symbols) for row in current]
